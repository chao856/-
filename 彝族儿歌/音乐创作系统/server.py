...
def generate_lyrics():
    ...
    lyric_generator = pipeline('text-generation', model='models/yi_folk_lyrics_model')  # 假设模型存储在models目录下的微调版本
    # 验证模型加载状态
...
@app.route('/arrange_chords', methods=['POST'])
def arrange_chords():
    data = request.get_json()
    ...
    for line in lyrics.split('\n'):
        if line.strip():
            chord_progression.append(random.choice(chords))
    
    return jsonify({'chords': '\n'.join([f"{chord}: {line}" for chord, line in zip(chord_progression, lyrics.split('\n'))])})
